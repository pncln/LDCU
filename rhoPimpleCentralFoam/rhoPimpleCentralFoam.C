...
int main(int argc, char *argv[])
{
    ...
    while (runTime.run())
    {
        ...
        {
            // Compute Co on LDCU flux without polluting scope with LDCU fields
            scalar CoNum = 0.0;
            scalar meanCoNum = 0.0;
            {
                // Compute LDCU flux early in a local scope for Co only
                #include "computeLdcuPhi.H"
                phiTurb = phiLDCU;

                scalarField sumPhi
                (
                    fvc::surfaceSum(mag(phiLDCU))().primitiveField()/rho.primitiveField()
                );
                CoNum = 0.5*gMax(sumPhi/mesh.V().field())*runTime.deltaTValue();
                meanCoNum =
                    0.5*(gSum(sumPhi)/gSum(mesh.V().field()))*runTime.deltaTValue();
                Info<< "Courant Number (LDCU) mean: " << meanCoNum
                    << " max: " << CoNum << endl;
            }
            #include "setDeltaT.H"
        }
        ...
        while (pimple.loop())
        {
            ...
            // Compute central-upwind mass flux used for all convective terms
            // (recompute here after any mesh motion)
            #include "computeLdcuPhi.H"
            phiTurb = phiLDCU;
